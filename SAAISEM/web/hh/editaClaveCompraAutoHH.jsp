...
<%
    ...
    String folio_gnk = "", fecha = "", folio_remi = "", orden = "", provee = "", recib = "", entrega = "", origen = "", coincide = "", observaciones = "", clave = "", descrip = "", cod_bar = "", um = "", lote = "", cadu = "", cajas = "", piezas = "", tarimas = "", marca = "", fec_fab = "", proveedor = "", tarimasInc = "", CostoU = "", ordenSuministro = "", MarcaComercial = "", unidadFonsabi = "", uniFon = "";
    int PzxCaja = 0, tarimasC = 0, tarimasI = 0, cajasPorTarimaC = 0, cajasPorTarimaI = 0, resto = 0, cajasI = 0, cantidadPedido = 0, cantidadCompra = 0, contarnomco = 0;

    try {
        con.conectar();
        ResultSet rset = con.consulta("select F_PzaCaja, F_ClaPro, F_Lote, F_FecCad, F_FecFab, F_Marca, F_Cb, F_Cajas, F_Pz, F_Tarimas, F_Resto, F_TarimasI, F_CajasI, F_FolRemi, F_Provee, F_Costo, F_OrdenSuministro, F_Origen, F_OrdCom, F_MarcaComercial, F_unidadFonsabi from tb_compratemp where F_IdCom = '" + ((String) sesion.getAttribute("id")) + "' ");
        while (rset.next()) {
            folio_gnk = rset.getString("F_FolRemi");
            fecha = (String) session.getAttribute("fecha");
            folio_remi = rset.getString("F_FolRemi");
            orden = rset.getString("f_OrdCom");
            ...
            CostoU = rset.getString("F_Costo");
            PzxCaja = rset.getInt("F_PzaCaja");
            ordenSuministro = rset.getString("F_OrdenSuministro");
            MarcaComercial = rset.getString("F_MarcaComercial");
            unidadFonsabi = rset.getString("F_unidadFonsabi");
            ...
            ResultSet rsetUnidad = con.consulta("SELECT * FROM tb_uniatn WHERE F_ClaCli = '" + unidadFonsabi + "'");
            while (rsetUnidad.next()){
                uniFon = rsetUnidad.getString(2);
            }
            ...
            ResultSet rsetCompra = con.consulta("SELECT Sum(ct.F_Pz) AS cantCompra FROM tb_compratemp AS ct WHERE ct.F_OrdCom = '" + orden + "' AND ct.F_ClaPro = '" + clave + "' AND CT.F_IdCom <> '" + ((String) sesion.getAttribute("id")) + "' GROUP BY ct.F_ClaPro");
            while (rsetCompra.next()) {
                cantidadCompra = rsetCompra.getInt("cantCompra");
                System.out.println("cantidadCompra :" + cantidadCompra);
            }

            ResultSet rsetNomComer = con.consulta("select count(*) from tb_nombrecomercial where F_ClaPro =  '" + clave + "';");
            while (rsetNomComer.next()) {
                contarnomco = rsetNomComer.getInt(1);
            ...
            tarimasC = Integer.parseInt(tarimas);
            tarimasI = Integer.parseInt(tarimasInc);
            cajasPorTarimaC = Integer.parseInt(cajas) / tarimasC;
...
<html>
    ...
    <body onload="totalPiezas();">
        <div class="container">
            ...
            <form class="form-horizontal" role="form" name="formulario1" id="formulario1" method="get" action="../ModificacionesHH">
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-sm-1">
                            <input type="origen" class="form-control" id="origen" name="origen" placeholder="origen" readonly value="<%=(origen)%>">
                        </div>                            
                        <div class="col-sm-2">
                ...
                <div class="panel-footer">
                    ...
                    <div class="row">
                        ...
                        <label for="Marca" class="col-sm-1 control-label">Nombre comercial</label>
                        <div class="col-sm-2">
                            <input type="Marca" class="form-control" id="NomComer" name="Marca" readonly="" placeholder="Marca" onKeyPress="return tabular(event, this)" value="<%=MarcaComercial%>" />
                        </div>
                    ...
                    <br/>
                    <div class="row">
                        <label for="Caducidad" class="col-sm-1 control-label">Caducidad</label>
                        <div class="col-sm-2">
                        ...
                        <label for="Caducidad" class="col-sm-1 control-label">Costo U.</label>
                        <div class="col-sm-2">
                            <input type="text" readonly="readonly" class="form-control" id="CostoU" name="CostoU" placeholder="Costo Unitario" onKeyPress="return tabular(event, this)" value="<%=CostoU%>" />
                        </div>
                        ...
                        <% if (origen.equals("4") || origen.equals("5") || origen.equals("10") || origen.equals("11") || origen.equals("14") || origen.equals("15") || origen.equals("16") || origen.equals("17") || origen.equals("18") ) {%>
                        <label for="ordenSuministro" class="col-sm-1 control-label">Orden de Suministro</label>
                        <div class="col-sm-3">
                            <input type="text" maxlength="40" class="form-control" id="ordenSuministro" name="ordenSuministro" placeholder="Orden Suministro" onkeypress="return tabular(event, this)" value="<%=ordenSuministro%>">
                        </div>
                        <% } else if (origen.equals("19")) {%>
                        <label for="unidadFonsabi" class="col-sm-1 control-label"> Unidad</label>
                        <div class="col-sm-2">                            
                            <select class="form-control" name="unidadFonsabi" id="unidadFonsabi" onkeypress="return  tabular(event, this)">
                                <% if(unidadFonsabi.equals("S/U")){ %> 
                                <option value="S/U">Sin conocimiento de unidad destino</option>
                                <% } else {%>                                
                                <option value="<%=unidadFonsabi%>"><%=uniFon%></option>
                                <option value="S/U">Sin conocimiento de unidad destino</option>
                                <% } %>
                            <%                                            
                                try {
                        ...
                        <div class="col-sm-0">
                            <input type="hidden" class="form-control" id="cantidadPedido" name="cantidadPedido" value="<%=cantidadPedido%>" />
                        </div>
                        <div class="col-sm-0">
                            <input type="hidden" class="form-control" id="cantidadCompra" name="cantidadCompra" value="<%=cantidadCompra%>" />
                        </div>
                        ...
                        <br/>
                        <!-- En duda -->
                        <button class="btn btn-block btn-success" type="submit" name="accion" value="actualizarCompraAuto" onclick="return (validaCapturaVacios());">Actualizar</button>
                        <!-- En duda -->
                    </div>
            </form>
        </div>
...
<script>
    ...
    %>
                            ...
                            function validaCapturaVacios() {
                                ...
                                if (origen === '4' || origen === '5' || origen === '10' || origen === '11' || origen === '14' || origen === '15' || origen === '16' || origen === '19') {
                                ...
                                var cantPedido = document.getElementById('cantidadPedido').value;
                                console.log(cantPedido);
                                var cantCompra = document.getElementById('cantidadCompra').value;
                                console.log(cantCompra);
                                var pza = document.getElementById('Piezas').value;
                                pza = pza.replace(/,/gi, "");
                                console.log(pza);
                                var sumIngreso = parseInt(cantCompra) + parseInt(pza);
                                console.log("suma del ingreso" + sumIngreso);
                                if (sumIngreso > parseInt(cantPedido)) {
                                    alert("Excede la cantidad a recibir, favor de verificar");
                                    return false;
                                } else if (parseInt(pza) <= 0) {
