...
<%
    ...
    String ClaPro = "", F_ClaUbi2 = "", UbiCb = "", CbUbicacion = "";
    try {
            ...
            <form action="leerInsRedistCBUbica.jsp" method="post">
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <input class="form-control" name="F_ClaUbi2" id="F_ClaUbi2"  placeholder="AA-1-1" />


                    </div>
                    <div class="col-sm-6"> 
                        <label>CB Ubicación</label>
                        <input class="form-control" name="F_CbUbicacion" id="F_CbUbicacion" placeholder="010101" />
                    </div>
                </div>
                <br/>
                <div class="row">
                    <div class="col-lg-12">
                        <button class="btn btn-block btn-success btn-lg" type="submit" name="UbiAnt" value="Leer">Leer Ubicación</button>
                    </div>
                </div>
            </form>
            <hr/>
                        ...
                        <tbody>
                            <%
                                try {
                                    ...
                                    if (F_ClaUbi2 != "") {
                                        rset = con.consulta("SELECT u.F_DesUbi, l.F_ClaPro, l.F_ExiLot, m.F_DesPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y') AS F_FecCad, l.F_IdLote, l.F_Cb, l.F_Origen, p.F_DesProy, o.F_DesOri,u.F_ClaUbi FROM tb_lote l INNER JOIN tb_medica m ON l.F_ClaPro = m.F_ClaPro INNER JOIN tb_ubica u ON l.F_Ubica = u.F_ClaUbi INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id INNER JOIN tb_origen o ON l.F_Origen = o.F_ClaOri WHERE l.F_ExiLot != 0 AND u.F_DesUbi = '" + F_ClaUbi2 + "' AND l.F_Ubica NOT IN (" + UbiCrossd + ");");
                                    } else if (CbUbicacion != "") {
                                        rset = con.consulta("SELECT u.F_DesUbi, l.F_ClaPro, l.F_ExiLot, m.F_DesPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y') AS F_FecCad, l.F_IdLote, l.F_Cb, l.F_Origen, p.F_DesProy, o.F_DesOri,u.F_ClaUbi FROM tb_lote l INNER JOIN tb_medica m ON l.F_ClaPro = m.F_ClaPro INNER JOIN tb_ubica u ON l.F_Ubica = u.F_ClaUbi INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id INNER JOIN tb_origen o ON l.F_Origen = o.F_ClaOri WHERE l.F_ExiLot != 0 AND u.F_Cb = '" + CbUbicacion + "' AND l.F_Ubica NOT IN (" + UbiCrossd + ");");
                                    }
                                    ...
                                    <form action="ingCantRedist.jsp" method="post">
                                        ...
                                        <%  if (!rset.getString("F_DesUbi").contains("MERMA-REDFRIA")) {
                                            if (rset.getString("F_DesUbi").contains("ACOPIO") || rset.getString("l.F_Origen").equals("14")) {
                                                if (usua.equals("Aureliano") || usua.equals("AGomezFa") || usua.equals("DcNJaen") || usua.equals("ACruzMo") || usua.equals("LMMarchan") || usua.equals("JRobles") || usua.equals("MPadua") || usua.equals("DanielH") || usua.equals("JEscutiaMu") || usua.equals("Ssaldana") || usua.equals("SHernandezMi") || usua.equals("GRamirezDi") || usua.equals("LFuentesEs")) { %>
