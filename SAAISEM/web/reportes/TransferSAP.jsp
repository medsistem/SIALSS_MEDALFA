...
response.setHeader("Content-Disposition","attachment; filename=TransferenciaSAP_"+F_ClaDoc+".xls");
...
<table border=0 cellpadding=0 cellspacing=0 width=439 style='border-collapse:
 ...
 <%
       ...
       con.conectar();
               ...
               rset2   = con.consulta("SELECT A.F_ClaDyn,M.F_ProMov,L.F_ClaLot,DATE_FORMAT(F_FecCad,'%d/%m/%Y') AS F_FecCad,SUM(F_CantMov) AS F_CantMov FROM tb_movinv M INNER JOIN tb_lote L ON M.F_LotMov=L.F_FolLot AND M.F_UbiMov=L.F_Ubica LEFT JOIN tb_artiis A ON M.F_ProMov=A.F_ClaInt  WHERE F_ConMov='"+ConInv+"' AND F_DocMov='"+F_ClaDoc+"' GROUP BY M.F_ProMov,L.F_ClaLot,F_FecCad");
           
  %>
   <tr height=20 style='height:15.0pt'>
  <td height=20 style='height:15.0pt;mso-ignore:colspan'>Clave Sap</td>
  <td class=xl66>Clave Gnkl</td>
