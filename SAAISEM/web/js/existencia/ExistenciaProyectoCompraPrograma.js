$(document).ready(function () {
    ...
    $('#Descargar').click(function () {
        ...
        window.open("../ExcelExistenciaProyectoCompraProgama?Proyecto=" + Proyecto + "&Tipo=Compra&Consulta=" + ConsultaT + "");
    });
...
function MostrarRegistros() {
    ...
    if (Consulta && Proyecto !== "0") {
        $.ajax({
            url: "../ExistenciaProyecto",
            data: {accion: "MostrarRegistrosCompraPrograma", Proyecto: Proyecto, Tipo: Consulta},
            type: 'GET',
            async: false,
            success: function (data)
            {
...
function limpiarTabla() {
    $("#ExistenciasComprasPrograma").remove();
}
...
function MostrarTabla(data) {
    ...
    for (var i = 0; i < json.length; i++) {
       ...
       // var fuentefinanciamento = json[i].fuentefinanciamento;
        ...
        switch (Tipo) {
              ...
              case "1":
                aDataSet.push([ ClaProSS, Descripcion, Cantidad]);
                break;
            case "2":
                aDataSet.push([ ClaProSS, Descripcion, Lote, Caducidad, Cantidad]);
                break;           
            default:
                aDataSet.push([ClaProSS, Descripcion, Cantidad]);
                break;    
    ...
    $(document).ready(function () {
        switch (Tipo) {
            case "1":
                $('#dynamic').html('<table class="table table-striped table-bordered table-condensed"  id="ExistenciasComprasPrograma"></table>');
                $('#ExistenciasComprasPrograma').dataTable({
