...
<%
    ...
    File archivo;
    archivo = new File("C:\\ABASTO\\ISEM\\Abasto_" + request.getParameter("F_ClaDoc") + "-" + request.getParameter("ConInv") + "-" + request.getParameter("idProyecto") + ".csv");
...
abasto.crearAbastoWeb(Integer.parseInt(Documento),  Integer.parseInt( Proyecto), usua);*/
        ...
        //String queryDatosCsV = "SELECT F.F_ClaCli, F.F_Proyecto, F.F_ClaDoc, LTRIM(RTRIM(F.F_ClaPro)), M.F_DesPro, LTRIM(RTRIM(L.F_ClaLot)), DATE_FORMAT(L.F_FecCad, '%d/%m/%Y') AS F_FecCad, SUM(F.F_CantSur), L.F_Origen, SUBSTR(L.F_Cb, 1, 13) AS F_Cb, NOW(), F.F_Lote as LOTE FROM tb_factura F INNER JOIN tb_lote L ON F.F_Lote = L.F_FolLot AND F.F_ClaPro = L.F_ClaPro AND F.F_Ubicacion = L.F_Ubica INNER JOIN tb_medica M ON F.F_ClaPro = M.F_ClaPro WHERE F_ClaDoc = '" + request.getParameter("F_ClaDoc") + "' AND F_CantSur > 0 AND F_StsFact = 'A' AND F.F_Proyecto = '" + request.getParameter("idProyecto") + "' GROUP BY F.F_ClaPro, L.F_ClaLot, L.F_FecCad, L.F_Origen;";
