...
<%
    ...
    response.setHeader("Content-Disposition", "attachment;filename=\"Reabastecer_Modula1.xls");
%>
<div>
                    ...
                    <%
                        try {
                            ...
                            try {
                                ...
                                ResultSet rset = con.consulta("SELECT M.F_Clapro,MD.F_DesPro,M.F_Min,M.F_Reo,M.F_Max,M.F_Ubi FROM tb_minmax M INNER JOIN tb_medica MD ON M.F_Clapro=MD.F_ClaPro WHERE M.F_Ubi NOT IN ('D','MODULA2','AF','MATCUR','CADUCADOS','LERMA','PROXACADUCAR','MERMA','CROSSDOCKMORELIA','INGRESOS_V','CUARENTENA') ORDER BY M.F_Clapro+0;");
                                while (rset.next()) {
                                    F_Max = rset.getInt(5);
                                    F_Reo = rset.getInt(4);
                                    F_Min = rset.getInt(3);
