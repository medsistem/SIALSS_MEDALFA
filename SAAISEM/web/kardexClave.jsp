    ...
    try {
        ...
        if (request.getParameter("accion").equals("Clave")) {
            ...
            while (rsetDesc.next()) {
                ...
                qryLote = ("select F_ClaPro, F_ClaLot, DATE_FORMAT(F_FecCad, '%d/%m/%Y') from tb_lote where F_ClaPro IN ('" + request.getParameter("Clave") + "') group by F_FecCad, F_ClaLot");
                CB = "";
            }
             con.cierraConexion();
        }
                ...
                <div class="panel-body">
                    <form name="FormKardex" action="kardexClave.jsp" method="Post">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-5">
                                <a class="btn btn-success form-control" a href="gnrKardexClave.jsp?Clave=<%=Clave%>&Lote=<%=request.getParameter("Lote")%>&Cadu=<%=request.getParameter("Cadu")%>&Btn=<%=request.getParameter("accion")%>&ProyectoCL=">Reporte de Trazabilidad</a>
                            </div>
                        </div>
                    </form>
                    <hr/>
                    ...
                    <h4>Caducidad: 
                        ...
                        <%
                            try {
                                con.conectar();
                                ResultSet rset = con.consulta("select F_DesPro from tb_medica where F_ClaPro IN ('" + Clave + "');");
                                while (rset.next()) {
                        ...
                        <%
                            try {
                                ...
                                ResultSet rset = null;
                                String ProyectoExit = "";
                                if ((!(Proyecto.equals("0"))) || (!(Proyecto.equals("")))) {
                                    ProyectoExit = " AND F_Proyecto='" + Proyecto + "' ";
                                }
                                if (request.getParameter("accion").equals("Clave")) {
                                    rset = con.consulta("select SUM(F_ExiLot) from tb_lote where F_ClaPro IN ('" + Clave + "')  and F_ExiLot !=0;");
                                } else {
                                    rset = con.consulta("select SUM(F_ExiLot) from tb_lote where F_ClaPro = '" + Clave + "' and F_ClaLot ='" + request.getParameter("Lote") + "' and F_FecCad = STR_TO_Date('" + request.getParameter("Cadu") + "', '%d/%m/%Y') and F_Origen = '" + request.getParameter("F_Origen") + "'  and F_ExiLot !=0;");
                                }
