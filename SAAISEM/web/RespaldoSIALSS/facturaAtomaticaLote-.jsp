...
<%
    ...
    String usua = "", Clave = "", Desproyecto = "", tipo = "", F_Ruta = "", F_FecEnt = "", Unidad1 = "", Unidad2 = "", Catalogo = "", Campo = "";
    ...
    try {
        ...
        ResultSet UbiMod = con.consulta("SELECT PU.F_Id, P.F_Id, IFNULL(P.F_DesProy, '') AS Proyecto, P.F_Campo FROM tb_parametrousuario PU LEFT JOIN ( SELECT F_Id, F_DesProy, F_Campo FROM tb_proyectos ) P ON PU.F_Proyecto = P.F_Id WHERE F_Usuario =  '" + usua + "';");
        if (UbiMod.next()) {
            UbicaModu = UbiMod.getInt(1);
            Proyecto = UbiMod.getInt(2);
            Desproyecto = UbiMod.getString(3);
            Campo = UbiMod.getString(4);
        }
...
<html>
    ...
    <body>
        ...
        <div class="container">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">Facturación aútomatica Lote Proyecto : <%=Desproyecto%></h3>
                </div>
                <div class="panel-body ">
                    <form class="form-horizontal" role="form" name="formulario1" id="formulario1" method="post" action="facturaAtomaticaLote.jsp">
                    ...
                    <form action="Facturacion" method="post" onsubmit="muestraImagen()">
                        <%                            int banReq1 = 0, banReq = 0;
                            try {
                                ...
                                banReq = 0;
                                int F_PiezasReq = 0, TotalSur = 0, UbicaModu2 = 0, banunidad = 0;
                                if (!(Catalogo == "")) {

                                    ResultSet rset = con.consulta("select  F_ClaCli AS F_ClaUni, F_NomCli from tb_uniatn where F_ClaCli between  '" + Unidad1 + "' and '" + Unidad2 + "'  AND F_Proyecto = '" + Proyecto + "' group by F_ClaCli;");
                                    while (rset.next()) {
                                        con.actualizar("UPDATE tb_unireqlote U LEFT JOIN ( SELECT F_ClaPro FROM tb_medica WHERE " + Campo + " = 1 ) AS M ON U.F_ClaPro = M.F_ClaPro SET U.F_Status = 1 WHERE U.F_ClaUni = '" + rset.getString(1) + "' AND U.F_Status = 0 AND M.F_ClaPro IS NULL;");
                                        banunidad = 1;
                                        F_NomCli = rset.getString("F_NomCli");
                                        ResultSet rset3 = con.consulta("select F_ClaUni, sum(F_Solicitado) as F_PiezasReq,DATE_FORMAT(F_Fecha,'%d/%m/%Y') AS F_Fecha from tb_unireqlote REQ INNER JOIN tb_medica M ON REQ.F_ClaPro=M.F_ClaPro AND M.F_N" + Catalogo + "='1' where F_Status = '0' and F_ClaUni = '" + rset.getString(1) + "' group by F_ClaUni;");
                                        while (rset3.next()) {
                                            banReq = 1;
                                            banReq1 = 1;
                                    ...
                                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered table-condensed" id="datosProv">
                                        ...
                                        <%
                                            try {
                                                ...
                                                ResultSet rsetR1 = null;
                                                
                                                    rsetR1 = con.consulta("SELECT M.F_ClaPro, SUBSTR(M.F_DesPro, 1, 80) AS F_DesPro, REQ.F_CajasReq, REQ.F_Solicitado, REPLACE (M.F_ClaPro, '.', ''), IFNULL(LOTE.F_ExiLot, 0),  IFNULL(LOTE.F_ExiLot, 0) AS EXITLOTE, REQ.F_Lote, REQ.F_Caducidad, REQ.F_IdReq FROM tb_unireqlote REQ INNER JOIN tb_medica M ON REQ.F_ClaPro=M.F_ClaPro LEFT JOIN (SELECT F_ClaPro, F_ClaLot, F_FecCad, SUM(F_ExiLot) AS F_ExiLot FROM tb_lote " + UbicaDesc + " AND F_Proyecto='" + Proyecto + "' AND F_FecCad >= DATE_ADD(CURDATE(), INTERVAL 1 YEAR) GROUP BY F_ClaPro, F_ClaLot, F_FecCad) AS LOTE ON REQ.F_ClaPro = LOTE.F_ClaPro AND REQ.F_Lote = LOTE.F_ClaLot AND REQ.F_Caducidad = LOTE.F_FecCad WHERE F_ClaUni='" + rset.getString("F_ClaUni") + "' and F_Status =0 and F_Solicitado != 0 AND F_N" + Catalogo + "='1' AND M.F_StsPro='A' group by F_IdReq order by M.F_ClaPro+0;");
                                                

                                                while (rsetR1.next()) {
                                                    ExiLot = rsetR1.getInt(7);
                                        %>
                                        <tr
                                            <%
                                            ...
                                            <td><%=rsetR1.getString(8)%></td>
                                            <td><%=rsetR1.getString(9)%></td>
                                            <td><%=rsetR1.getInt(4)%></td>
                                            <td ><small><input name="Cantidad_<%=rset.getString(1)%>_<%=rsetR1.getString(5).trim()%>_<%=rsetR1.getString(10)%>" id="Cantidad_<%=rset.getString(1)%>_<%=rsetR1.getString(5).trim()%>_<%=rsetR1.getString(10)%>" type="number" min="0" class="text-right form-control" value="<%=ExiSol%>" data-behavior="only-num" /></small></td>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <button class="btn btn-block btn-warning" type="submit" name="accion" value="cancelarLote" onclick="return validaRemision()">Cancelar Folio(s)</button> 
                            </div>
                            <div class="col-sm-6">
        ...
        <script src="js/facturajs/AutomaticaFacturacionLote.js"></script>
        ...
        <script>
                                    ...
                                    function eliminarReq(e) {
                                        ...
                                        if (confirma) {
                                            ...
                                            $.ajax({
                                                type: 'POST',
                                                url: 'Facturacion?eliminarLote=' + F_ClaUni,
                                                /*data: form.serialize(),*/
