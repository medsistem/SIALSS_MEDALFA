...
<%
    ...
    int Proyecto = 0, UbicaModu = 0, CantCauses = 0;
    ...
    try {
        ...
        ResultSet RsCauses = con.consulta("SELECT SUM(F_Cause) FROM tb_catalogoprecios WHERE F_Proyecto = '" + Proyecto + "';");
        if (RsCauses.next()) {
            CantCauses = RsCauses.getInt(1);
        }
...
<html>
    ...
    <body>
        ...
        <div class="container">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">Facturación aútomatica Cause Proyecto : <%=Desproyecto%></h3>
                </div>
                <div class="panel-body ">
                    <form class="form-horizontal" role="form" name="formulario1" id="formulario1" method="post" action="facturaAtomaticaCause.jsp">
                ...
                <div class="panel-footer">
                    <form action="Facturacion" method="post" onsubmit="muestraImagen()">
                            ...
                            try {
                                ...
                                if (!(Catalogo == "")) {
                                    ...
                                    while (rset.next()) {
                                        ...
                                        ResultSet rset3 = con.consulta("SELECT F_ClaUni, sum(F_Solicitado) AS F_PiezasReq, DATE_FORMAT(F_Fecha, '%d/%m/%Y') AS F_Fecha FROM tb_unireq REQ INNER JOIN tb_medica M ON REQ.F_ClaPro = M.F_ClaPro INNER JOIN tb_catalogoprecios C ON REQ.F_ClaPro = C.F_ClaPro WHERE M.F_N" + Catalogo + "='1' AND F_Status = '0' and F_ClaUni = '" + rset.getString(1) + "' AND C.F_Proyecto = '" + Proyecto + "'  group by F_ClaUni;");
                                        while (rset3.next()) {
                                            banReq = 1;
                                            banReq1 = 1;
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <button class="btn btn-block btn-warning" type="submit" name="accion" value="cancelarCause" onclick="return validaRemision()">Cancelar Folio(s)</button>
                            </div>
                            <div class="col-sm-6">
        ...
        <script src="js/facturajs/AutomaticaFacturacionCause.js"></script>
        ...
        <script>
                                    ...
                                    function validaRemision() {
                                        var confirmacion = confirm('Seguro que desea cancelar los Folios');
