                ...
                <div class="panel-body">
                    <form name="FormKardex" action="kardexClaveCompras.jsp" method="Post">
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                <button class="btn btn-success" name="accion" value="CB">Por Código de Barras</button>
                            </div> 
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3">
                                <button class="btn btn-success" name="accion" value="Buscar">Buscar</button>
                                <%
                                    /*try {
                                        if (!request.getParameter("Lote").equals("")) {*/
                                %>
                                <a class="btn btn-success" a href="gnrKardexClave.jsp?Clave=<%=Clave%>&Lote=<%=request.getParameter("Lote")%>&Cadu=<%=request.getParameter("Cadu")%>&Btn=<%=request.getParameter("accion")%>">Reporte de Trazabilidad</a>
                                <%
                                    /* }
                                    } catch (Exception e) {
                    ...
                    <h4>Caducidad: 
                        ...
                        <%
                            try {
                                ...
                                ResultSet rset = null;
                                if (request.getParameter("accion").equals("Clave")) {
                                    rset = con.consulta("select SUM(F_ExiLot) from tb_lote where F_ClaPro = '" + Clave + "'  and F_ExiLot !=0");
                                } else {
                                ...
                                <tbody>
                                    <%
                                        try {
                                            ...
                                            ResultSet rset = null;
                                            if (request.getParameter("accion").equals("Clave")) {
                                                rset = con.consulta("SELECT m.F_User, m.F_ConMov, c.F_DesCon, m.F_ProMov, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), (m.F_CantMov * m.F_SigMov), m.F_CostMov, u.F_DesUbi, DATE_FORMAT(m.F_FecMov, '%d/%m/%Y'), m.F_hora, m.F_DocMov, m.F_IdMov, m.F_LotMov, m.F_UbiMov FROM tb_movinv m INNER JOIN tb_coninv c ON m.F_ConMov = c.F_IdCon INNER JOIN tb_ubica u ON m.F_UbiMov = u.F_ClaUbi INNER JOIN tb_lote l ON m.F_LotMov = l.F_FolLot AND m.F_ProMov = l.F_ClaPro AND m.F_UbiMov = l.F_Ubica WHERE m.F_ProMov = '" + Clave + "' and m.F_ConMov!=1000 GROUP BY m.F_IdMov ORDER BY m.F_IdMov;");
                                            } else {
                                                rset = con.consulta("SELECT m.F_User, m.F_ConMov, c.F_DesCon, m.F_ProMov, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), (m.F_CantMov * m.F_SigMov), m.F_CostMov, u.F_DesUbi, DATE_FORMAT(m.F_FecMov, '%d/%m/%Y'), m.F_hora, m.F_DocMov, m.F_IdMov FROM tb_movinv m INNER JOIN tb_coninv c ON m.F_ConMov = c.F_IdCon INNER JOIN tb_ubica u ON m.F_UbiMov = u.F_ClaUbi INNER JOIN tb_lote l ON m.F_LotMov = l.F_FolLot AND m.F_ProMov = l.F_ClaPro AND m.F_UbiMov = l.F_Ubica WHERE m.F_ProMov = '" + Clave + "' and l.F_ClaLot ='" + request.getParameter("Lote") + "' and l.F_FecCad=STR_TO_Date('" + request.getParameter("Cadu") + "', '%d/%m/%Y') and F_Origen = '" + request.getParameter("F_Origen") + "' and m.F_ConMov!=1000 GROUP BY m.F_IdMov ORDER BY m.F_IdMov");
                                            }
