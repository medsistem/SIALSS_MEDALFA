        ...
        try {
            ...
            switch (Consulta) {
                case "1":
                    ...
                    rowHeadInv.createCell((int) 1).setCellValue("Nombre Generico");
                ...
                case "3":
                    ...
                    rowHeadInv.createCell((int) 9).setCellValue("Bodega");
                    rowHeadInv.createCell((int) 10).setCellValue("Fuente Financiamiento");
                    for (int i = 0; i <= 10; i++) {
                        rowHeadInv.getCell(i).setCellStyle(style);
                    }
                    break;
            ...
            if (Proyecto.equals("0")) {
                ...
                switch (Consulta) {
                    case "1":
                        qry = "SELECT P.F_DesProy, L.F_ClaPro, M.F_DesPro, L.F_ClaLot, DATE_FORMAT(L.F_FecCad, '%d/%m/%Y') AS F_FecCad, SUM(L.F_ExiLot) AS F_ExiLot, O.F_DesOri, L.F_Ubica,M.F_PrePro,tb_proveedor.F_ClaProve,tb_proveedor.F_NomPro, b.Nombre_Bodega as lugar, IFNULL(M.F_NomGen,'') AS NomGen  FROM tb_lote L INNER JOIN tb_medica M ON L.F_ClaPro = M.F_ClaPro INNER JOIN tb_origen O ON L.F_Origen = O.F_ClaOri INNER JOIN tb_proyectos P ON L.F_Proyecto = P.F_Id INNER JOIN tb_proveedor ON tb_proveedor.F_ClaProve = L.F_ClaPrv LEFT JOIN tb_ubica as u ON u.F_ClaUbi=L.F_Ubica LEFT JOIN tb_bodegas AS b ON b.Id_Bodega = u.Id_Bodega WHERE L.F_Proyecto = '" + Proyecto + "' AND L.F_ExiLot>0 " + AND + " AND L.F_ClaPro NOT IN ('9999', '9998', '9996', '9995') AND L.F_Origen <> '19' AND u.Estatus=1 AND b.Estatus=1 GROUP BY L.F_ClaPro, L.F_Proyecto;";
                        break;
                    case "2":
                        qry = "SELECT P.F_DesProy, L.F_ClaPro, M.F_DesPro, L.F_ClaLot, DATE_FORMAT(L.F_FecCad, '%d/%m/%Y') AS F_FecCad, SUM(L.F_ExiLot) AS F_ExiLot, O.F_DesOri, L.F_Ubica,M.F_PrePro,tb_proveedor.F_ClaProve,tb_proveedor.F_NomPro, b.Nombre_Bodega as lugar, IFNULL(M.F_NomGen,'') AS NomGen FROM tb_lote L INNER JOIN tb_medica M ON L.F_ClaPro = M.F_ClaPro INNER JOIN tb_origen O ON L.F_Origen = O.F_ClaOri INNER JOIN tb_proyectos P ON L.F_Proyecto = P.F_Id INNER JOIN tb_proveedor ON tb_proveedor.F_ClaProve = L.F_ClaPrv LEFT JOIN tb_ubica as u ON u.F_ClaUbi=L.F_Ubica LEFT JOIN tb_bodegas AS b ON b.Id_Bodega = u.Id_Bodega WHERE L.F_Proyecto = '" + Proyecto + "' AND L.F_ExiLot>0 " + AND + " AND L.F_ClaPro NOT IN ('9999', '9998', '9996', '9995') AND L.F_Origen <> '19'  AND u.Estatus=1 AND b.Estatus=1 GROUP BY L.F_ClaPro, L.F_ClaLot, L.F_FecCad, L.F_Origen, L.F_Proyecto,tb_proveedor.F_NomPro;";
                        break;
                    case "3":
                        qry = "SELECT P.F_DesProy, L.F_ClaPro, M.F_DesPro, L.F_ClaLot, DATE_FORMAT(L.F_FecCad, '%d/%m/%Y') AS F_FecCad, SUM(L.F_ExiLot) AS F_ExiLot, O.F_DesOri, L.F_Ubica,M.F_PrePro,tb_proveedor.F_ClaProve,tb_proveedor.F_NomPro, b.Nombre_Bodega as lugar, '' AS F_FuenteFinanza, IFNULL(M.F_NomGen,'') AS NomGen  FROM tb_lote L INNER JOIN tb_medica M ON L.F_ClaPro = M.F_ClaPro INNER JOIN tb_origen O ON L.F_Origen = O.F_ClaOri INNER JOIN tb_proyectos P ON L.F_Proyecto = P.F_Id INNER JOIN tb_proveedor ON tb_proveedor.F_ClaProve = L.F_ClaPrv LEFT JOIN tb_ubica as u ON u.F_ClaUbi=L.F_Ubica LEFT JOIN tb_bodegas AS b ON b.Id_Bodega = u.Id_Bodega WHERE L.F_Proyecto = '" + Proyecto + "' AND L.F_ExiLot>0 " + AND + " AND L.F_ClaPro NOT IN ('9999', '9998', '9996', '9995') AND L.F_Origen <> '19' AND u.Estatus=1 AND b.Estatus=1 GROUP BY L.F_ClaPro, L.F_ClaLot, L.F_FecCad, L.F_Origen, L.F_Proyecto, L.F_Ubica;";
                        break;
                    default:
                        break;
                }
