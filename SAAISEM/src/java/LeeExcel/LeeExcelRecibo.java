        ...
        try {
            ...
            while (BuscaPP2.next()) {
                ...
                if (!BuscaPP2.wasNull()) {
                    ...
                    if (!(Contrato.equals("") && Contrato.equals(null))) {
                        if (NoOrden.contains("CCISEM-AR-2020-") || NoOrden.contains("OCISEM-AR-2020-")) { // valida el nombre la=da la oc
...
//                            con.actualizar("UPDATE tb_cargaoc O INNER JOIN tb_proveedor P ON O.F_Proveedor = P.F_ClaProve INNER JOIN tb_prodprov pp ON O.F_Proveedor = pp.F_ClaProve SET O.F_ProblemaProvee= 1 WHERE O.F_UsU='" + User + "';");
//                            con.actualizar("UPDATE tb_cargaoc O INNER JOIN tb_medica M ON O.F_Clave = M.F_ClaPro  INNER JOIN tb_prodprov pp ON O.F_Clave = pp.F_ClaPro AND O.F_Proveedor = pp.F_ClaProve SET O.F_ProblemaClave= 1 WHERE O.F_Usu='" + User + "';");
//                            con.actualizar("UPDATE tb_cargaoc O INNER JOIN tb_medica M ON O.F_Clave = M.F_ClaPro AND M.F_StsPro = 'S' SET O.F_ProblemaClave= 0 WHERE O.F_Usu='" + User + "';");
...
//                            con.actualizar("UPDATE tb_cargaoc O SET O.F_ProblemaReq = 1 WHERE O.F_Usu ='" + User + "';");
////                con.actualizar("UPDATE tb_cargaoc O INNER JOIN tb_prodprov pp ON  O.F_Clave = pp.F_ClaPro AND  O.F_Cant >= pp.F_CantMin SET O.F_ProblemaReq = 1 AND O.F_Proveedor = pp.F_ClaProve WHERE O.F_Usu ='" + User + "';");
                        } else {
                            System.out.println("Sin ar");
                            // con.actualizar("DELETE FROM tb_cargaoc WHERE  F_Usu='" + User + "';");
...
//                            con.actualizar("UPDATE tb_cargaoc O INNER JOIN tb_medica M ON O.F_Clave = M.F_ClaPro SET O.F_ProblemaClave= 1 WHERE O.F_Usu='" + User + "';");
//                            con.actualizar("UPDATE tb_cargaoc O INNER JOIN tb_medica M ON O.F_Clave = M.F_ClaPro AND M.F_StsPro = 'S' SET O.F_ProblemaClave= 0 WHERE O.F_Usu='" + User + "';");
//                            con.actualizar("UPDATE tb_cargaoc O INNER JOIN tb_proyectos P ON O.F_Proyecto = P.F_Id SET O.F_ProblemaProyecto = 1 WHERE O.F_Usu='" + User + "';");
               ...
               if ((NoReg == NoClave) && (NoReg == NoProvee) && (NoReg == NoProyecto) && (NoReg == NoOc) && (NoReg == NoCantidad)) {
                    ...
                    ResultSet Claves = con.consulta("SELECT F_NoOc, P.F_ClaProve, F_Clave, F_Clavess, SUM(F_Cant) AS F_Cant,F_Proyecto, O.F_FuenteFinanza,O.F_Contratos FROM tb_cargaoc O INNER JOIN tb_proveedor P ON O.F_Proveedor = P.F_ClaProve INNER JOIN tb_proyectos PR ON O.F_Proyecto=PR.F_Id INNER JOIN tb_medica M ON O.F_Clave = M.F_ClaPro AND  M.F_StsPro != 'S' WHERE F_Usu = '" + User + "' AND F_NoOc ='" + Consulta.getString(1) + "' AND P.F_ClaProve = '" + Consulta.getString(2) + "' AND F_Proyecto='" + Consulta.getString(10) + "' AND o.F_Cant > 0 GROUP BY O.F_Id");
                        ...
                        while (usuarioc.next()) {
                            ...
                            ResultSet FechaSur = con.consulta("select DATE_ADD(curdate(), INTERVAL 7 DAY)");
                            String FechaS = "";
                            while (FechaSur.next()) {
...
//                                con.insertar("INSERT INTO tb_pedido_sialss VALUES(0,'" + Claves.getString(1) + "','" + Claves.getString(2) + "','" + Claves.getString(3) + "','" + Claves.getString(4) + "','-','1','-',CURDATE(),'" + Claves.getString(5) + "','-',NOW(),'" + FechaS + "',CURTIME(),'" + usuc + "',1,0,'" + Claves.getString(6) + "','','CC/LP','" + Claves.getString(7) + "',0,'')");
                    ...
                    out.println("<script>window.location='cargarecibo.jsp'</script>");
                }
            } else {
                         out.println("<script>alert('No se pudo cargar, verifique las celdas')</script>");
                    out.println("<script>window.location='cargarecibo.jsp'</script>");     
